...
import { handleBilboMDAutoMDSaxsJob } from './handleBilboMDAutoMDSaxsJob.js'
import { getEnvVar } from '../../config/config.js'

const uploadFolder = path.join(getEnvVar('DATA_VOL'))

const createAutoMDSaxsJob = async (req: Request, res: Response) => {
  const UUID = uuid()
  const jobDir = path.join(uploadFolder, UUID)
  logger.info(`createAutoMDSaxsJob ${UUID}`)
  try {
...
const dispatchAutoMDSaxsJob = async (ctx: BilboMDDispatchContext) => {
  const { req, res, bilbomd_mode, user, UUID, accessMode, publicId, client_ip_hash } =
    ctx
  logger.info(`Starting AutoMD-SAXS Job mode: ${bilbomd_mode} (${accessMode})`)
  await handleBilboMDAutoMDSaxsJob(req, res, user, UUID, {
    accessMode,
    publicId,
    client_ip_hash
  })
